...
emailInput.onblur = () => {
    if (!emailInput.value.includes("@")) {
        errStr += "El correo ha de incluir un '@'\n";
    }
    if (!emailInput.value.includes(".")) {
        errStr += "El correo ha de incluir un '.'\n";
    }
...
passInput.onblur = () => {
    if (passInput.value.length != 8) {
        errStr += "La contraseña ha de tener un mínimo de 8 carácteres\n";
    }
    if (!/^[A-Za-z]+$/.test(passInput.value)) {
        errStr += "La contraseña ha de contener una letra mayúscula y una minúscula\n";
    }
    if (!/^\d+$/.test(passInput.value)) {
        errStr += "La contraseña ha de contener un número\n";
    }
}
rePassInput.onblur = () => {
